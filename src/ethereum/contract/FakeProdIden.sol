...
contract FakeProdIden
{
    ...
    function sellToSeller(string memory prodId, string memory sellerid) public 
    {
        ...
        require(allSellersInfo[sellerid].exist == true);
    ...
    {
        allSellersInfo[sellerId] = SellerInfo(sellerId, true);
    }
    ...
    function productVerification(string memory prodId, uint32 consumerCode) public view returns(bool) 
    {
        ...
        if(hasReachedToSeller[prodId].hasReached == true  && productStorage[prodId].consumerCode == consumerCode)
        {
            return true;
        }
        else
